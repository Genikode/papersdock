...
const ENDPOINTS = {
  ...
  REPLY_CREATE: '/query/student/reply',
...
function PdfCard({ url, filename }: { url: string; filename?: string }) {
  return (
    <div className="flex items-center gap-2 sm:gap-3 rounded-lg bg-[#f0fff4] p-2 sm:p-3 ring-1 ring-emerald-100 dark:bg-emerald-950 dark:ring-emerald-900">
      <div className="flex h-[56px] w-[72px] sm:h-[70px] sm:w-[88px] items-center justify-center rounded-md bg-white ring-1 ring-emerald-100 dark:bg-slate-900 dark:ring-emerald-900">
        <FileText className="h-5 w-5 sm:h-6 sm:w-6 text-emerald-600 dark:text-emerald-400" />
      </div>
      <div className="min-w-0 flex-1">
        <div className="truncate text-xs sm:text-sm font-medium text-emerald-900 dark:text-emerald-100">
          {filename || url.split('/').pop() || 'Document.pdf'}
        </div>
        <div className="mt-0.5 text-[11px] sm:text-xs text-emerald-700 dark:text-emerald-300">PDF • Tap to view</div>
        <a
          href={url}
          target="_blank"
          rel="noreferrer"
          className="mt-1 sm:mt-2 inline-flex items-center gap-1 text-[11px] sm:text-xs font-medium text-emerald-700 hover:underline dark:text-emerald-300"
        >
...
function VoiceBubble({ url }: { url: string }) {
  ...
  return (
    <div className="flex items-center gap-2 sm:gap-3 rounded-xl bg-[#e7f7ff] p-2 sm:p-3 ring-1 ring-sky-200 dark:bg-sky-950 dark:ring-sky-900">
      <div className="flex h-8 w-8 sm:h-9 sm:w-9 items-center justify-center rounded-full bg-white ring-1 ring-sky-200 dark:bg-slate-900 dark:ring-sky-900">
        <FileAudio className="h-4 w-4 sm:h-5 sm:w-5 text-sky-600 dark:text-sky-300" />
      </div>
      <button
        onClick={() => { const a = audioRef.current; if (!a) return; if (playing) { a.pause(); setPlaying(false); } else { a.play(); setPlaying(true); } }}
        className="inline-flex h-8 w-8 sm:h-9 sm:w-9 items-center justify-center rounded-full bg-sky-600 text-white dark:bg-sky-400 dark:text-slate-900"
      >
      ...
      <div className="relative mx-1 h-7 sm:h-8 flex-1 min-w-[120px] sm:min-w-[140px] max-w-[220px] sm:max-w-[320px] overflow-hidden">
        <div className="absolute inset-0 flex items-end gap-[2px] sm:gap-[3px]">
          {Array.from({ length: 36 }).map((_, i) => (
            <div key={i} className="w-[3px] sm:w-[4px] rounded-t bg-sky-400 dark:bg-sky-500" style={{ height: `${(Math.sin(i / 2) * 0.5 + 0.5) * 14 + 3}px` }} />
          ))}
        </div>
        <div className="pointer-events-none absolute inset-y-0 left-0 bg-sky-100 dark:bg-sky-800" style={{ width: `${Math.round(progress * 100)}%` }} />
      </div>
...
export default function ViewQueryPage() {
  ...
  /* ================================ UI ================================ */
  const infoCard = (
    <aside className="top-0 h-full border bg-white p-6 shadow-[0_1px_0_#eceef1] dark:bg-slate-950 dark:border-slate-800">
      <h2 className="text-base font-semibold text-slate-900 dark:text-slate-100">Query Information</h2>
      <div className="mt-4 rounded-md border p-3 dark:border-slate-800">
        <div className="text-[15px] font-semibold text-slate-900 dark:text-slate-100">{loadingQuery ? 'Loading…' : q?.title || '—'}</div>
        {q?.text && <p className="mt-2 text-sm text-slate-700 dark:text-slate-300">{q.text}</p>}
      </div>
      <dl className="mt-4 space-y-3 text-sm">
        <div className="flex items-start gap-2">
          <BookOpen className="mt-0.5 h-4 w-4 text-slate-500 dark:text-slate-400" />
          <div>
            <dt className="text-slate-500 dark:text-slate-400">Paper</dt>
            <dd className="font-medium text-slate-900 dark:text-slate-100">{courseTitle}</dd>
          </div>
        </div>
        <div className="flex items-start gap-2">
          <BookOpen className="mt-0.5 h-4 w-4 text-slate-500 dark:text-slate-400" />
          <div>
            <dt className="text-slate-500 dark:text-slate-400">Chapter</dt>
            <dd className="font-medium text-slate-900 dark:text-slate-100">{chapterTitle}</dd>
          </div>
        </div>
        <div className="flex items-start gap-2">
  ...
  const headerBar = (
    <header className="top-0 z-10 border-b dark:border-slate-800">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-3 sm:px-6 lg:px-8">
        <div className="flex items-center gap-2">
          <button
            onClick={() => router.back()}
            className="rounded-md border px-2.5 py-2 text-slate-700 hover:bg-slate-50 dark:text-slate-200 dark:border-slate-700 dark:hover:bg-slate-800"
          >
            <ArrowLeft className="h-4 w-4" />
          </button>
          <h1 className="text-base font-semibold text-slate-900 sm:text-lg dark:text-slate-100">Query Details</h1>
        </div>
        {q && (
  ...
  return (
    <div className="min-h-screen min-w-s bg-slate-50 dark:bg-slate-900">
      {headerBar}
      ...
      <main className="mx-auto grid max-w-7xl gap-3 sm:gap-4 px-3 py-4 sm:px-6 lg:grid-cols-[1fr_340px] lg:gap-5 lg:px-8">
        {/* Conversation */}
        <section className="order-2 lg:order-1 relative flex min-h-[60vh] flex-col rounded-lg border bg-white p-2.5 sm:p-4 shadow-[0_1px_0_#eceef1] dark:bg-slate-950 dark:border-slate-800">
          <h2 className="text-sm sm:text-base font-semibold text-slate-900 dark:text-slate-100">Conversation</h2>
          ...
          <div className="mt-2 sm:mt-3 flex-1 space-y-3 sm:space-y-4 overflow-y-auto overflow-x-hidden overscroll-contain pr-1 pb-28 sm:pb-24">
            ...
            <div className={clsx('flex w-full items-start gap-2 justify-end')}>
              <div
                className={clsx(
                  'min-w-0 max-w-[82%] sm:max-w-[85%] md:max-w-[70%] rounded-xl p-2.5 sm:p-3 shadow-sm ring-1 bg-white ring-slate-200 dark:bg-slate-950 dark:ring-slate-700'
                )}
              >
                <div className={clsx('mb-1 text-[10px] sm:text-[11px] break-words text-slate-400 dark:text-slate-500')}>
                  <span className="font-medium">{'You'}</span> • {timeShort(q?.createdAt)}
                </div>

                <div className="mt-4 rounded-md">
                  {q?.text && <p className="mt-2 text-sm text-black dark:text-slate-100">{q.text}</p>}
            ...
            {loadingReplies && <div className="text-xs sm:text-sm text-slate-500 dark:text-slate-400">Loading replies…</div>}
            {!loadingReplies && replies.length === 0 && (
              <div className="text-xs sm:text-sm text-slate-500 dark:text-slate-400">No replies yet.</div>
            )}
            ...
            {!loadingReplies && replies.map((m, idx) => {
              ...
              return (
                ...
                >
                  {!mine && (
                    <div className="mt-5 sm:mt-6 hidden h-6 w-6 sm:h-7 sm:w-7 shrink-0 rounded-full bg-slate-200 ring-1 ring-slate-300 sm:block dark:bg-slate-700 dark:ring-slate-600" />
                  )}
                  ...
                  <div
                    className={clsx(
                      ...
                      mine
                        ? 'bg-white ring-slate-200 dark:bg-slate-950 dark:ring-slate-700'
                        : 'bg-slate-900 text-white ring-slate-800 dark:bg-slate-800 dark:ring-slate-700'
                    )}
                  >
                    <div
                      className={clsx(
                        'mb-1 text-[10px] sm:text-[11px] break-words',
                        mine ? 'text-slate-400 dark:text-slate-500' : 'text-slate-300 dark:text-slate-200'
                      )}
                    >
                    ...
                    {m.text && (
                      <p
                        className={clsx(
                          'text-[13px] sm:text-sm break-words whitespace-pre-wrap',
                          mine ? 'text-slate-800 dark:text-slate-100' : 'text-white dark:text-slate-100'
                        )}
                      >
                        {m.text}
                      </p>
                    ...
                    {m.attachmentUrl && isPdf && <div className="mt-2"><PdfCard url={m.attachmentUrl} /></div>}
                  ...
                  {mine && (
                    <div className="mt-5 sm:mt-6 hidden h-6 w-6 sm:h-7 sm:w-7 shrink-0 rounded-full bg-emerald-100 ring-1 ring-emerald-200 sm:block dark:bg-emerald-900/40 dark:ring-emerald-800" />
                  )}
                </div>
              );
            })}
          ...
          {/* Composer */}
          <div className="sticky bottom-0 w-full bg-white pt-2 pb-[env(safe-area-inset-bottom)] dark:bg-slate-950">
            {(rec.recording || rec.blob) && (
              <div className="mb-2 flex flex-wrap items-center justify-between gap-2 rounded-lg border border-slate-200 bg-white px-2.5 sm:px-3 py-1.5 sm:py-2 shadow-sm dark:border-slate-700 dark:bg-slate-900">
                <div className="flex min-w-0 items-center gap-2 text-rose-600 dark:text-rose-400">
                  <span className="inline-block h-2 w-2 animate-pulse rounded-full bg-rose-600 dark:bg-rose-400" />
                  ...
                  <div className="ml-2 hidden items-end gap-[2px] sm:gap-[3px] sm:flex">
                    {Array.from({ length: 24 }).map((_, i) => (
                      <div
                        key={i}
                        className="w-[2px] sm:w-[3px] rounded-t bg-rose-300 dark:bg-rose-500"
                ...
                <div className="flex shrink-0 items-center gap-1.5 sm:gap-2">
                  {rec.recording && (
                    <button
                      type="button"
                      onClick={() => rec.pause()}
                      className="rounded-md border border-slate-200 p-1.5 sm:p-2 text-slate-700 hover:bg-slate-50 dark:text-slate-200 dark:border-slate-700 dark:hover:bg-slate-800"
            ...
            <div className="flex items-center gap-1.5 sm:gap-2 rounded-xl border border-slate-200 bg-white p-1.5 sm:p-2 shadow-sm dark:border-slate-700 dark:bg-slate-900">
              <input
                ref={fileRef}
                type="file"
              ...
              <button
                ...
                className="inline-flex shrink-0 items-center justify-center rounded-md border border-slate-200 p-1.5 sm:p-2 text-slate-700 hover:bg-slate-50 dark:text-slate-200 dark:border-slate-700 dark:hover:bg-slate-800"
              ...
              <input
                ...
                className="min-w-0 flex-1 rounded-md bg-transparent px-2 py-1.5 sm:py-2 text-[13px] sm:text-sm outline-none placeholder:text-slate-400 dark:text-slate-100 dark:placeholder:text-slate-500"
              />
              ...
              <button
                type="button"
                onClick={handleSend}
                className="inline-flex shrink-0 items-center justify-center rounded-md bg-slate-900 p-1.5 sm:p-2 text-white hover:opacity-95 disabled:opacity-60 dark:bg-slate-200 dark:text-slate-900"
            ...
            {file && (
              <div className="mt-2 inline-flex max-w-full items-center gap-2 rounded border bg-white px-2 py-1 text-[11px] sm:text-xs text-slate-600 dark:bg-slate-900 dark:border-slate-700 dark:text-slate-300">
